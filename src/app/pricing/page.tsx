...
const PricingPage = async () => {
	// NOTE: These are simply fetching from server
	// NOTE: if using useQuery, it will be updated real-time
	const user = await currentUser() // INFO: This is a Clerk user
	const convex = new ConvexHttpClient(process.env.NEXT_PUBLIC_CONVEX_URL!)
	const convexUser = await convex.query(api.users.getUser, {
		userId: user?.id || "",
	}) // INFO: This is a Convex user -- come from the Convex schema (database)
				...
				<div className="max-w-7xl mx-auto">
					...
					<div className="mt-24 relative">
						<div className="flex flex-col items-center justify-center gap-6">
							<div
								className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-500/10
             text-blue-400  text-sm mb-2"
							>
								<Shield className="size-4 text-blue-400" />
								24/7 Privacy Support
							</div>

							<div className="inline-flex items-center gap-2 px-4 py-1.5 text-gray-400 text-sm">
								Out dedicated team is here to help you succeed
							</div>
						</div>
					</div>
				</div>
