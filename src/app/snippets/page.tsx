...
const Snippets = () => {
	...
	const [debouncedSearch] = useDebounce(searchQuery, 500) // NOTE: Re-renders only after 500ms of user stopping typing for 500ms
	...
	/* javascript -> (wait 500ms) -> render */

	const [view, setView] = useState<"grid" | "list">("grid")
	const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null)

	// CHECKPOINT: Check for loading state
	...
	// NOTE: the "snippets" variable can contain multiple snippets with the same language
	// NOTE: So, we need to filter out the unique languages from the snippets
	// INFO: Sets only store unique values
	// INFO: Spread operator converts Set back to array --> Results in array of unique languages
	const languages = [...new Set(snippets.map((snippet) => snippet.language))]
	const popularLanguages = languages.slice(0, 5)

	const filteredSnippets = snippets.filter((snippet) => {
		// INFO: empty string.includes("") is always true
		const matchesSearch =
		...
		// INFO: !selectedLanguage is true (when selectedLanguage is empty)
			...
			<div className="relative max-w-7xl mx-auto px-4 py-12">
             ...
             from-blue-500/10 to-purple-500/10 text-sm text-gray-400 mb-6"
                  ...
                  placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
					...
					<div className="flex flex-wrap items-center gap-4">
						...
						{popularLanguages.map((lang) => (
							<button
								key={lang}
								onClick={() => setSelectedLanguage(lang === selectedLanguage ? null : lang)} // INFO: Toggle language -- if already selected, clear it -- else, set it
								className={`
				...
				<motion.div
					...
					layout // NOTE: Animate layout changes
				>
